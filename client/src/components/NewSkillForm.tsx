...
const NewSkillForm: React.FC = (): React.ReactElement => {
    ...
    const onSubmit: React.FormEventHandler<HTMLFormElement> = async (
        ...
        try {
            const { data } = await axios.post(
                "http://192.168.1.64:3001/skills/create",
                DataForm,
                header
            );

            if (data.SkillCreated) {
    ...
    return (
        <form className={form_styles.form} onSubmit={onSubmit}>
            ...
            {skillCreated && <InfoCard title="Skill Created" colour="green" />}
        </form>
    );
};
